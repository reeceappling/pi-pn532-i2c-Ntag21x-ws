...
var (
	...
	// Really really bad errors that should never happen
	ErrBadSessionInStorage = errors.New("non-session found in storage. Should never happen")
)
...
func (sm *Map[T, U]) GetSession(id U) utils.Result[Session[T]] {
	...
	if !ok {
		return utils.ErroredResult[Session[T]](ErrBadSessionInStorage)
	}
	if sess.Expiry.Before(time.Now()) {
		sm.sessions.Delete(id)
		return utils.ErroredResult[Session[T]](ErrSessionExpired)
	}
